...
printf "\n\n"    >> abi.in
grep -A3 rprim ../abi.GS | tail -n 4 >> abi.in
...
k=$(       grep ngkpt    ../abi.GS | awk '{print $2}')
ecut=$(    grep 'ecut\b' ../abi.GS | awk '{print $2}')
nband=$(   grep nband    ../abi.GS | awk '{print $2}')
diemac=$(  grep diemac   ../abi.GS | awk '{print $2}')
nshiftk=$( grep nshiftk  ../abi.GS | awk '{print $2}')

grep '^nshiftk' ../abi.GS >> abi.in
grep -A $nshiftk '^shiftk' ../abi.GS >> abi.in
...
grep ecutsm     ../abi.GS  >> abi.in
grep ngfft      ../abi.GS  >> abi.in
grep ^pawecutdg ../abi.GS  >> abi.in
grep ^pawxcdev  ../abi.GS  >> abi.in
...
if [ -f abi-i_DS1_DEN ] && [ -f abi-i_DS1_WFK ]
then
   sed -i 's/irdden1.*/irdden1 1/g' abi.in
   sed -i 's/irdwfk1.*/irdwfk1 1/g' abi.in
fi
