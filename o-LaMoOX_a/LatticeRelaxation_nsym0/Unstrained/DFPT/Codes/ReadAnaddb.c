...
for(i=0;i<natom;i++){
   Sortoms[i].Z[0][0]=Atoms[i].Z[0][0]; Sortoms[i].Z[0][1]=Atoms[i].Z[0][1]; Sortoms[i].Z[0][2]=Atoms[i].Z[0][2];
   Sortoms[i].Z[1][0]=Atoms[i].Z[1][0]; Sortoms[i].Z[1][1]=Atoms[i].Z[1][1]; Sortoms[i].Z[1][2]=Atoms[i].Z[1][2];
   Sortoms[i].Z[2][0]=Atoms[i].Z[2][0]; Sortoms[i].Z[2][1]=Atoms[i].Z[2][1]; Sortoms[i].Z[2][2]=Atoms[i].Z[2][2];
   strcpy(Sortoms[i].name, Atoms[i].name);
   }
...
while(swp){
   ...
   for(i=0;i<natom-1;i++){
	  ...
	  for(j=0;j<3;j++)
	     for(k=0;k<3;k++){
		    if( fabs(Max1) < fabs(Sortoms[i].Z[j][k])   )
		       Max1=Sortoms[i].Z[j][k];
		    if( fabs(Max2) < fabs(Sortoms[i+1].Z[j][k]) )
		       Max2=Sortoms[i+1].Z[j][k];
		    }
      
      if( fabs(Max1) < fabs(Max2) ){
	     tempAt=Sortoms[i];
	     Sortoms[i]=Sortoms[i+1];
	     Sortoms[i+1]=tempAt;
	     count++;
	     } 	   
      }
...
if(print==1){
   for(i=0;i<natom;i++){
      printf("%6s %3d %10.4lf %10.4lf %10.4lf\n",Sortoms[i].name, 1, Sortoms[i].Z[0][0], Sortoms[i].Z[0][1], Sortoms[i].Z[0][2]);
      printf("%6s %3d %10.4lf %10.4lf %10.4lf\n",Sortoms[i].name, 2, Sortoms[i].Z[1][0], Sortoms[i].Z[1][1], Sortoms[i].Z[1][2]);
      printf("%6s %3d %10.4lf %10.4lf %10.4lf\n",Sortoms[i].name, 3, Sortoms[i].Z[2][0], Sortoms[i].Z[2][1], Sortoms[i].Z[2][2]);
      }

   double Zx, Zy, Zz, Zxy, Zxz, Zyz, Zyx, Zzx, Zzy;
   Zx=Zy=Zz=Zxy=Zxz=Zyz=Zyx=Zzx=Zzy=0.0;
   for(i=0;i<natom;i++){
      Zx+= Atoms[i].Z[0][0];
      Zy+= Atoms[i].Z[1][1];
      Zz+= Atoms[i].Z[2][2];
